...
public class TrafficSystemEditor : Editor
{
    ...
    private string[]
        m_trafficSystemRoadPiecesLocation; /*         = {  "Assets/Traffic System/Prefabs/Road Pieces/L2_Straight.prefab",
...
//	float myFloat = 1.23f;
    ...
    static public string PrimaryNodeLeftSideMaterial =
        "Assets/Traffic System/Materials/Traffic System Primary Node Left.mat";

    static public string PrimaryNodeRightSideMaterial =
        "Assets/Traffic System/Materials/Traffic System Primary Node Right.mat";

    static public string SecondaryNodeLeftSideMaterial =
        "Assets/Traffic System/Materials/Traffic System Secondary Node Left.mat";

    static public string SecondaryNodeRightSideMaterial =
        "Assets/Traffic System/Materials/Traffic System Secondary Node Right.mat";
...
//	}
    ...
    void Awake()
    {
        ...
        if (!TrafficSystem)
            Debug.LogError(
                "Traffic System Error -> \"Traffic System\" script must be in the scene. It is missing. Drop in the \"Traffic System\" Prefab to fix this.");
        ...
        if (TrafficSystem)
        {
            TrafficSystem.TextureIconAnchor =
                AssetDatabase.LoadAssetAtPath("Assets/Traffic System/Textures/icon_anchor.png", typeof(Texture2D)) as
                    Texture2D;
            TrafficSystem.TextureIconEdit =
                AssetDatabase.LoadAssetAtPath("Assets/Traffic System/Textures/icon_edit.png", typeof(Texture2D)) as
                    Texture2D;
        }
...
//	}
    ...
    public void ChangeRoadQuality()
    {
        switch (TrafficSystem.Instance.RoadQualityElementsIndex)
        {
            case 0:
                ...
                {
                    string file = m_trafficSystemRoadPiecesLocation[aIndex].Substring(m_dirRoadPieces.Length,
                        m_trafficSystemRoadPiecesLocation[aIndex].Length - m_dirRoadPieces.Length - 7);
                    TrafficSystemPiece roadPiece =
                        AssetDatabase.LoadAssetAtPath(m_trafficSystemRoadPiecesLocation[aIndex],
                            typeof(TrafficSystemPiece)) as TrafficSystemPiece;
    ...
    public override void OnInspectorGUI()
    {
        ...
        TrafficSystem.Instance.RoadQualityElementsIndex =
            EditorGUILayout.Popup(TrafficSystem.Instance.RoadQualityElementsIndex, m_roadQualityElements);
...
//			TrafficSystem.Instance.m_quickSpawn      = GUILayout.Toggle(TrafficSystem.Instance.m_quickSpawn, " Quick Spawn");
            ...
            for (int aIndex = 0; aIndex < m_trafficSystemRoadPiecesLocation.Length; aIndex++)
            {
                ...
                if (GUILayout.Button(file))
                {
                    ...
                    if (roadPiece)
                    {
                        TrafficSystemPiece roadPieceClone =
                            PrefabUtility.InstantiatePrefab(roadPiece) as TrafficSystemPiece;
                        if (roadPieceClone)
                        {
...
//							}
                            ...
                            roadPieceClone.transform.localScale =
                                TrafficSystem.m_roadScale * roadPieceClone.transform.localScale;
                            ...
                            {
                                SetTrafficSystemPiece(TrafficSystem.TrafficSystemTooltip.ANCHOR,
                                    EditTrafficSystemPiece);
                                FirstNode = true;
                            }
        ...
        else
        {
            ...
            {
                GUILayout.BeginHorizontal("box");
                TrafficSystem.Instance.m_swapAnchorDimensions =
                    GUILayout.Toggle(TrafficSystem.Instance.m_swapAnchorDimensions,
                        " Flip 'Anchor' Renderer Size     .");
                TrafficSystem.Instance.m_swapEditDimensions =
                    GUILayout.Toggle(TrafficSystem.Instance.m_swapEditDimensions, " Flip 'Edit' Renderer Size");
                GUILayout.FlexibleSpace();
                GUILayout.EndHorizontal();
                GUILayout.BeginHorizontal("box");
//				TrafficSystem.Instance.m_swapOffsetDir        = GUILayout.Toggle(TrafficSystem.Instance.m_swapOffsetDir,        " Flip Offset Dir                        .");
                TrafficSystem.Instance.m_swapOffsetSize = GUILayout.Toggle(TrafficSystem.Instance.m_swapOffsetSize,
                    " Flip Offset Size (x -> z, z -> x)      .");
                TrafficSystem.Instance.m_negateOffsetSize = GUILayout.Toggle(TrafficSystem.Instance.m_negateOffsetSize,
                    " Negate Offset Size (-x, -y, -z)");
                GUILayout.FlexibleSpace();
                GUILayout.EndHorizontal();
            ...
            if (EditTrafficSystemPiece != AnchorTrafficSystemPiece)
            {
                ...
                for (int bIndex = 0; bIndex < EditTrafficSystemPiece.m_leftLaneChangeNodes.Count; bIndex++)
                {
                    ...
                    if (node && node.m_isInbound)
                    {
                        ...
                        {
                            CreateAllChangeLaneLinks(true, TrafficSystem.DriveSide.LEFT, (int) node.m_changeLaneID,
                                false, true);
                        }
                ...
                for (int bIndex = 0; bIndex < EditTrafficSystemPiece.m_rightLaneChangeNodes.Count; bIndex++)
                {
                    ...
                    if (node && node.m_isInbound)
                    {
                        ...
                        if (GUILayout.Button("Reveal ->"))
                        {
                            CreateAllChangeLaneLinks(true, TrafficSystem.DriveSide.RIGHT, (int) node.m_changeLaneID,
                                false, true);
                        }
                ...
                for (int bIndex = 0; bIndex < AnchorTrafficSystemPiece.m_leftLaneChangeNodes.Count; bIndex++)
                {
                    ...
                    if (node && node.m_isOutbound)
                    {
                        ...
                        if (GUILayout.Button("Reveal ->"))
                        {
                            CreateAllChangeLaneLinks(false, TrafficSystem.DriveSide.LEFT, (int) node.m_changeLaneID,
                                true, true);
                        }
                ...
                for (int bIndex = 0; bIndex < AnchorTrafficSystemPiece.m_rightLaneChangeNodes.Count; bIndex++)
                {
                    ...
                    if (node && node.m_isOutbound)
                    {
                        ...
                        if (GUILayout.Button("Reveal ->"))
                        {
                            CreateAllChangeLaneLinks(false, TrafficSystem.DriveSide.RIGHT, (int) node.m_changeLaneID,
                                true, true);
                        }
...
//		EditorGUILayout.EndToggleGroup ();
        ...
        if (TrafficSystem && TrafficSystem.Instance)
        {
            ...
            if (!TrafficSystem.Instance.TooltipAnchor)
            {
                if (ResourceNodeFolder && ResourceNodeFolder.childCount > 1)
                {
                    ...
                    {
                        if (ResourceNodeFolder.GetChild(cIndex) &&
                            ResourceNodeFolder.GetChild(cIndex).name == "Tooltip-Anchor")
                ...
                {
                    GameObject toolTip =
                        AssetDatabase.LoadAssetAtPath(TooltipAnchorLocation, typeof(GameObject)) as GameObject;
            ...
            if (!TrafficSystem.Instance.TooltipEdit)
            {
                if (ResourceNodeFolder && ResourceNodeFolder.childCount > 1)
                {
                    ...
                    {
                        if (ResourceNodeFolder.GetChild(cIndex) &&
                            ResourceNodeFolder.GetChild(cIndex).name == "Tooltip-Edit")
                            if (ResourceNodeFolder.GetChild(cIndex).gameObject)
                                DestroyImmediate(ResourceNodeFolder.GetChild(cIndex).gameObject);
                    }
                }
                ...
                {
                    GameObject toolTip =
                        AssetDatabase.LoadAssetAtPath(TooltipEditLocation, typeof(GameObject)) as GameObject;
            ...
            {
                SetTrafficSystemPiece(TrafficSystem.TrafficSystemTooltip.EDIT,
                    TrafficSystem.Instance.EditTrafficSystemPiece);
            ...
            {
                SetTrafficSystemPiece(TrafficSystem.TrafficSystemTooltip.ANCHOR,
                    TrafficSystem.Instance.AnchorTrafficSystemPiece);
    ...
    void CreateOppositeLinks(bool a_anchorToEdit, bool a_revealOnly = false)
    {
        if (EditTrafficSystemPiece != AnchorTrafficSystemPiece)
        {
            ...
            if (a_anchorToEdit)
            {
                CreateConnections(EditTrafficSystemPiece.m_primaryLeftLaneNodes,
                    AnchorTrafficSystemPiece.m_primaryRightLaneNodes, false, 0, a_revealOnly, true);
                CreateConnections(EditTrafficSystemPiece.m_secondaryLeftLaneNodes,
                    AnchorTrafficSystemPiece.m_primaryRightLaneNodes, false, 0, a_revealOnly, true, false);
                CreateConnections(AnchorTrafficSystemPiece.m_primaryLeftLaneNodes,
                    EditTrafficSystemPiece.m_primaryRightLaneNodes, false, 0, a_revealOnly, true, false);
                CreateConnections(AnchorTrafficSystemPiece.m_primaryLeftLaneNodes,
                    EditTrafficSystemPiece.m_secondaryRightLaneNodes, false, 0, a_revealOnly, true, false);
            }
            else
            {
                CreateConnections(EditTrafficSystemPiece.m_primaryRightLaneNodes,
                    AnchorTrafficSystemPiece.m_primaryLeftLaneNodes, false, 0, a_revealOnly, true);
                CreateConnections(EditTrafficSystemPiece.m_secondaryRightLaneNodes,
                    AnchorTrafficSystemPiece.m_primaryLeftLaneNodes, false, 0, a_revealOnly, true, false);
                CreateConnections(AnchorTrafficSystemPiece.m_primaryRightLaneNodes,
                    EditTrafficSystemPiece.m_primaryLeftLaneNodes, false, 0, a_revealOnly, true, false);
                CreateConnections(AnchorTrafficSystemPiece.m_primaryRightLaneNodes,
                    EditTrafficSystemPiece.m_secondaryLeftLaneNodes, false, 0, a_revealOnly, true, false);
            }
        }
    }
    ...
    void CreateAllLinks(bool a_anchorToEdit, bool a_revealOnly = false, bool a_linkSameLaneOnly = false)
    {
        ...
        if (EditTrafficSystemPiece != AnchorTrafficSystemPiece)
        {
                ...
                IsOffRampPiece(EditTrafficSystemPiece.m_primaryRightLaneNodes))
            {
                if (a_anchorToEdit)
                {
                    CreateOffRampConnection(AnchorTrafficSystemPiece.m_primaryLeftLaneNodes,
                        EditTrafficSystemPiece.m_primaryLeftLaneNodes, a_revealOnly);
                    CreateOffRampConnection(EditTrafficSystemPiece.m_primaryRightLaneNodes,
                        AnchorTrafficSystemPiece.m_primaryRightLaneNodes, a_revealOnly);
                }
                else
                {
                    CreateOffRampConnection(EditTrafficSystemPiece.m_primaryLeftLaneNodes,
                        AnchorTrafficSystemPiece.m_primaryLeftLaneNodes, a_revealOnly);
                    CreateOffRampConnection(AnchorTrafficSystemPiece.m_primaryRightLaneNodes,
                        EditTrafficSystemPiece.m_primaryRightLaneNodes, a_revealOnly);
                }
            }
            else
            {
                if (a_anchorToEdit)
                {
                    CreateConnections(AnchorTrafficSystemPiece.m_primaryLeftLaneNodes,
                        EditTrafficSystemPiece.m_primaryLeftLaneNodes, false, 0, a_revealOnly, true, false,
                        a_linkSameLaneOnly);
                    CreateConnections(AnchorTrafficSystemPiece.m_secondaryLeftLaneNodes,
                        EditTrafficSystemPiece.m_primaryLeftLaneNodes, false, 0, a_revealOnly, true, false,
                        a_linkSameLaneOnly);
                    CreateConnections(EditTrafficSystemPiece.m_primaryRightLaneNodes,
                        AnchorTrafficSystemPiece.m_primaryRightLaneNodes, false, 0, a_revealOnly, true, false,
                ...
                else
                {
                    ...
                    CreateConnections(AnchorTrafficSystemPiece.m_primaryRightLaneNodes,
                        EditTrafficSystemPiece.m_primaryRightLaneNodes, false, 0, a_revealOnly, true, false,
                        a_linkSameLaneOnly);
                    CreateConnections(AnchorTrafficSystemPiece.m_secondaryRightLaneNodes,
                        EditTrafficSystemPiece.m_primaryRightLaneNodes, false, 0, a_revealOnly, true, false,
                        a_linkSameLaneOnly);
                }
            }
    ...
    void CreateLeftLinks(bool a_anchorToEdit, bool a_revealOnly = false)
    {
        if (EditTrafficSystemPiece != AnchorTrafficSystemPiece)
        {
            ...
            {
                CreateConnections(AnchorTrafficSystemPiece.m_primaryLeftLaneNodes,
                    EditTrafficSystemPiece.m_primaryLeftLaneNodes, false, 0, a_revealOnly, true);
                CreateConnections(AnchorTrafficSystemPiece.m_secondaryLeftLaneNodes,
                    EditTrafficSystemPiece.m_primaryLeftLaneNodes, false, 0, a_revealOnly, true, false);
            }
            else
    ...
    void CreateRightLink(bool a_anchorToEdit, bool a_revealOnly = false)
    {
        if (EditTrafficSystemPiece != AnchorTrafficSystemPiece)
        {
            ...
            {
                CreateConnections(AnchorTrafficSystemPiece.m_primaryRightLaneNodes,
                    EditTrafficSystemPiece.m_primaryRightLaneNodes, false, 0, a_revealOnly, true);
                CreateConnections(AnchorTrafficSystemPiece.m_secondaryRightLaneNodes,
                    EditTrafficSystemPiece.m_primaryRightLaneNodes, false, 0, a_revealOnly, true, false);
            }
            else
    ...
    void CreateAllChangeLaneLinks(bool a_inbound, TrafficSystem.DriveSide a_side, int a_id, bool a_switch,
        bool a_revealOnly = false)
    {
        TrafficSystem.ClearCLRevealObjsFrom();
        TrafficSystem.ClearCLRevealObjsTo();
        ...
        if (EditTrafficSystemPiece != AnchorTrafficSystemPiece)
        {
            if (a_inbound)
            {
                ...
                {
                    if (a_switch)
                        CreateCLInBoundConnections(AnchorTrafficSystemPiece.m_primaryRightLaneNodes,
                            EditTrafficSystemPiece.m_leftLaneChangeNodes, a_id, a_revealOnly);
                    else
                        CreateCLInBoundConnections(AnchorTrafficSystemPiece.m_primaryLeftLaneNodes,
                            EditTrafficSystemPiece.m_leftLaneChangeNodes, a_id, a_revealOnly);
                }
                else
                {
                    if (a_switch)
                        CreateCLInBoundConnections(AnchorTrafficSystemPiece.m_primaryLeftLaneNodes,
                            EditTrafficSystemPiece.m_rightLaneChangeNodes, a_id, a_revealOnly);
                    else
                        CreateCLInBoundConnections(AnchorTrafficSystemPiece.m_primaryRightLaneNodes,
                            EditTrafficSystemPiece.m_rightLaneChangeNodes, a_id, a_revealOnly);
                }
            ...
            else
            {
                ...
                {
                    if (a_switch)
                        CreateCLOutBoundConnections(AnchorTrafficSystemPiece.m_leftLaneChangeNodes,
                            EditTrafficSystemPiece.m_primaryLeftLaneNodes, a_id, a_revealOnly);
                    else
                        CreateCLOutBoundConnections(AnchorTrafficSystemPiece.m_leftLaneChangeNodes,
                            EditTrafficSystemPiece.m_primaryRightLaneNodes, a_id, a_revealOnly);
                }
                else
                {
                    if (a_switch)
                        CreateCLOutBoundConnections(AnchorTrafficSystemPiece.m_rightLaneChangeNodes,
                            EditTrafficSystemPiece.m_primaryRightLaneNodes, a_id, a_revealOnly);
                    else
                        CreateCLOutBoundConnections(AnchorTrafficSystemPiece.m_rightLaneChangeNodes,
                            EditTrafficSystemPiece.m_primaryLeftLaneNodes, a_id, a_revealOnly);
    ...
    void CreateOffRampConnection(List<TrafficSystemNode> a_nodesLookingToConnect,
        List<TrafficSystemNode> a_nodesToConnectWith, bool a_revealOnly = false)
    {
    ...
    void CreateConnections(List<TrafficSystemNode> a_nodesLookingToConnect,
        List<TrafficSystemNode> a_nodesToConnectWith, bool a_isOutbound = false, int a_nodeID = 0,
        bool a_revealOnly = false, bool a_useConnectedNodeArray = false, bool a_clearReveal = true,
        bool a_linkSameLaneOnly = false)
    {
        if (a_clearReveal)
        {
            TrafficSystem.ClearCLRevealObjsFrom();
        ...
        for (int lNIndex = 0; lNIndex < a_nodesLookingToConnect.Count; lNIndex++)
        {
            ...
            for (int cNIndex = 0; cNIndex < a_nodesToConnectWith.Count; cNIndex++)
            {
                ...
                if (!nodeFound)
                {
                    if (a_linkSameLaneOnly ||
                        (currentNode.m_onlyConnectWithSameLane || lastNode.m_onlyConnectWithSameLane))
                    {
    ...
    void CreateCLInBoundConnections(List<TrafficSystemNode> a_nodesLookingToConnect,
        List<TrafficSystemNode> a_nodesToConnectWith, int a_nodeID, bool a_revealOnly = false)
    {
        for (int lNIndex = 0; lNIndex < a_nodesLookingToConnect.Count; lNIndex++)
        {
            TrafficSystemNode lastNode = a_nodesLookingToConnect[lNIndex];
...
//					}
    ...
    void CreateCLOutBoundConnections(List<TrafficSystemNode> a_nodesLookingToConnect,
        List<TrafficSystemNode> a_nodesToConnectWith, int a_nodeID, bool a_revealOnly = false)
    {
    ...
    {
        SetTrafficSystemPiece(TrafficSystem.TrafficSystemTooltip.EDIT,
            EditTrafficSystemPiece); // force reposition of edit icon
    }

    void PositionTrafficSystemPiece(TrafficSystemPiece a_currentPiece, TrafficSystemPiece a_attachToPiece,
        bool a_incIndex = true, bool a_useEditOffset = false, bool a_useAnchorOffset = false)
    {
        if (!a_currentPiece)
            return;
...
//			}
    ...
    void CheckForAnchorNode()
    {
        ...
        for (int cIndex = 0; cIndex < AnchorNodeFolder.childCount; cIndex++)
        {
            ...
            {
                SetTrafficSystemPiece(TrafficSystem.TrafficSystemTooltip.ANCHOR,
                    AnchorNodeFolder.GetChild(cIndex).GetComponent<TrafficSystemPiece>());
            }
        }
    }
