...
//		serializedObject.Update();
        ...
        if (TrafficSystemPiece.m_primaryLeftLaneNodes.Count > 0 ||
            TrafficSystemPiece.m_primaryRightLaneNodes.Count > 0 ||
            TrafficSystemPiece.m_leftLaneChangeNodes.Count > 0 || TrafficSystemPiece.m_rightLaneChangeNodes.Count > 0)
        {
                ...
                if (lastNode)
                {
                    ...
                    for (int cIndex = 0; cIndex < lastNode.m_connectedChangeLaneNodes.Count; cIndex++)
                    {
                        if (lastNode.m_connectedChangeLaneNodes[cIndex] &&
                            lastNode.m_connectedChangeLaneNodes[cIndex].Parent != TrafficSystemPiece)
                        {
                            string driverSide = "Pink";
                            ...
                            GUILayout.BeginVertical("box");
                            GUILayout.Label("ID: " + lastNode.m_connectedChangeLaneNodes[cIndex].m_changeLaneID + " " +
                                            driverSide + " CL Inbound  : " +
                                            lastNode.m_connectedChangeLaneNodes[cIndex].Parent);
                            GUILayout.Space(1.0f);
                            GUILayout.BeginHorizontal("box");
                            if (GUILayout.Button("Reveal ->"))
                            {
                                ...
                                TrafficSystem.ClearCLRevealObjsTo();
                                TrafficSystem.AddToCLRevealObjsFrom(lastNode.transform);
                                TrafficSystem.AddToCLRevealObjsTo(lastNode.m_connectedChangeLaneNodes[cIndex]
                                    .transform);
                            }
                ...
                if (lastNode)
                {
                    for (int cNIndex = 0; cNIndex < lastNode.m_connectedChangeLaneNodes.Count; cNIndex++)
                    {
                        ...
                        if (currentNode)
                        {
                            ...
                            GUILayout.BeginVertical("box");
                            GUILayout.Label((lNIndex + 1) + " " + driverSide + " CL Outbound   : " +
                                            currentNode.Parent);
                            GUILayout.Space(1.0f);
                            GUILayout.BeginHorizontal("box");
                            if (GUILayout.Button("Reveal ->"))
...
//			}
        ...
        if (TrafficSystem && TrafficSystem.TextureIconEdit)
        {
            if (GUILayout.Button(TrafficSystem.TextureIconEdit))
            {
                ...
                {
                    TrafficSystem.Instance.SetTrafficSystemPiece(TrafficSystem.TrafficSystemTooltip.EDIT,
                        TrafficSystemPiece);
        ...
        if (TrafficSystem && TrafficSystem.TextureIconAnchor)
        {
            if (GUILayout.Button(TrafficSystem.TextureIconAnchor))
            {
                if (TrafficSystem && TrafficSystem.Instance)
                {
                    TrafficSystem.Instance.SetTrafficSystemPiece(TrafficSystem.TrafficSystemTooltip.ANCHOR,
                        TrafficSystemPiece);
        ...
        if (TrafficSystemPiece.transform.childCount > 0 && TrafficSystem)
        {
            ...
            {
                if (TrafficSystemPiece.transform.GetChild(cIndex).GetComponent<TrafficSystemPiece>() &&
                    !TrafficSystemPiece.transform.GetChild(cIndex).GetComponent<TrafficSystemPiece>()
                        .m_ignoreTrafficSystemEditor)
                {
                    if (TrafficSystem && TrafficSystem.TextureIconEditRevealSmall)
                    {
                        if (GUILayout.Button(TrafficSystem.TextureIconEditRevealSmall))
                        {
                            ...
                            {
                                TrafficSystem.Instance.PositionTooltip(TrafficSystem.TrafficSystemTooltip.EDIT,
                                    TrafficSystemPiece.transform.GetChild(cIndex).GetComponent<TrafficSystemPiece>());
                                TrafficSystem.Instance.ShowTooltip(TrafficSystem.TrafficSystemTooltip.EDIT, true);
                            }
                        }
                    ...
                    if (TrafficSystem && TrafficSystem.TextureIconAnchorRevealSmall)
                    {
                        if (GUILayout.Button(TrafficSystem.TextureIconAnchorRevealSmall))
                        {
                            if (TrafficSystem.Instance)
                            {
                                TrafficSystem.Instance.PositionTooltip(TrafficSystem.TrafficSystemTooltip.ANCHOR,
                                    TrafficSystemPiece.transform.GetChild(cIndex).GetComponent<TrafficSystemPiece>());
                                TrafficSystem.Instance.ShowTooltip(TrafficSystem.TrafficSystemTooltip.ANCHOR, true);
                            }
                        }
...
//		GUILayout.EndHorizontal();
    ...
    public void RefreshEditor(bool a_children = true, bool a_refreshMaterials = true)
    {
        ...
        {
            Material material =
                AssetDatabase.LoadAssetAtPath(TrafficSystemEditor.PrimaryNodeLeftSideMaterial, typeof(Material)) as
                    Material;
        ...
        {
            Material material =
                AssetDatabase.LoadAssetAtPath(TrafficSystemEditor.PrimaryNodeRightSideMaterial, typeof(Material)) as
                    Material;
        ...
        {
            Material material =
                AssetDatabase.LoadAssetAtPath(TrafficSystemEditor.SecondaryNodeLeftSideMaterial, typeof(Material)) as
                    Material;
        ...
        {
            Material material =
                AssetDatabase.LoadAssetAtPath(TrafficSystemEditor.SecondaryNodeRightSideMaterial, typeof(Material)) as
                    Material;
    ...
    void OnSceneGUI()
    {
        ...
        for (int cIndex = 0; cIndex < TrafficSystemPiece.m_leftLaneChangeNodes.Count; cIndex++)
        {
            ...
            if (node)
            {
                ...
                if (!node.m_isInbound)
                    str = "Outbound";
                Handles.Label(node.transform.position + (Vector3.up * 0.5f),
                    "CL " + node.m_changeLaneID.ToString() + " ( Blue / " + node.m_driveSide + " - " + str + " )");
            }
        }
        ...
        for (int cIndex = 0; cIndex < TrafficSystemPiece.m_rightLaneChangeNodes.Count; cIndex++)
        {
            ...
            if (node)
            {
                ...
                if (!node.m_isInbound)
                    str = "Outbound";
                Handles.Label(node.transform.position + (Vector3.up * 0.5f),
                    "CL " + node.m_changeLaneID.ToString() + " ( Pink / " + node.m_driveSide + " - " + str + " )");
            }
        }
    ...
    public void CreateConnections(List<TrafficSystemNode> a_nodesLookingToConnect,
        List<TrafficSystemNode> a_nodesToConnectWith)
    {
    ...
    void ChangeNodeMaterial(TrafficSystemNode a_node, TrafficSystem.DriveSide a_side)
    {
        ...
        if (a_node.m_driveSide == TrafficSystem.DriveSide.LEFT)
        {
            ...
            {
                Material material =
                    AssetDatabase.LoadAssetAtPath(TrafficSystemEditor.SecondaryNodeLeftSideMaterial, typeof(Material))
                        as Material;
                a_node.GetComponent<Renderer>().material = material;
            }
        }
