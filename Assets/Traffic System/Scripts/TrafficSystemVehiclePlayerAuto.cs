...
public class TrafficSystemVehiclePlayerAuto : TrafficSystemVehicle
{
    ...
    public override void Update()
    {
        ...
        if (!StopMoving && !WaitForUserInput && TrafficLight && !CrashDetected
        ) // determine if there is there is room to move throught the intersection
        {
            ...
            else if (TrafficLight.m_status == TrafficSystemTrafficLight.Status.GREEN)
            {
                ...
                {
                    if (TrafficLight.m_intersection.m_intersectionSystem ==
                        TrafficSystemIntersection.IntersectionSystem.SINGLE)
                    {
                        //						print ("GO!");
                        TrafficLight = null;
                        StopMoving = false;
    ...
    void OnGUI()
    {
        ...
        if (m_nextNode.IsDirectionChange())
        {
            ...
            {
                if (GUI.Button(
                    new Rect(initScreenPosX + ((buttonSize.x + buttonSpacerX) * btnIndex), initScreenPosY, buttonSize.x,
                        buttonSize.y), "LEFT"))
                {
                    ...
                    if (m_nextNode.IsRoundabout())
                    {
                        ...
                        {
                            if (!node && m_nextNode.m_connectedChangeLaneNodes[cIndex].m_roundaboutExit ==
                                TrafficSystemNode.RoundaboutExit.EXIT_3)
                            {
                    ...
                    else
                    {
                        for (int cIndex = 0; cIndex < m_nextNode.m_connectedChangeLaneNodes.Count; cIndex++)
                        {
                            if (!node && m_nextNode.m_connectedChangeLaneNodes[cIndex].m_driveSide ==
                                TrafficSystem.DriveSide.LEFT)
                            {
                                node = m_nextNode.m_connectedChangeLaneNodes[cIndex];
                                break;
                            }
            ...
            if (m_nextNode.IsDirectionStraight())
            {
                        ...
                        buttonSize.y), "STRAIGHT"))
                {
                    ...
                    if (m_nextNode.IsRoundabout())
                    {
                        ...
                        {
                            if (!node && m_nextNode.m_connectedChangeLaneNodes[cIndex].m_roundaboutExit ==
                                TrafficSystemNode.RoundaboutExit.EXIT_2)
                            {
                                node = m_nextNode.m_connectedChangeLaneNodes[cIndex];
                                break;
                            }
            ...
            if (m_nextNode.IsDirectionRight())
            {
                        ...
                        buttonSize.y), "RIGHT"))
                {
                    ...
                    if (m_nextNode.IsRoundabout())
                    {
                        ...
                        {
                            if (!node && m_nextNode.m_connectedChangeLaneNodes[cIndex].m_roundaboutExit ==
                                TrafficSystemNode.RoundaboutExit.EXIT_1)
                            {
                                node = m_nextNode.m_connectedChangeLaneNodes[cIndex];
                                break;
                            }
            ...
            if (m_nextNode.IsDirectionUTurn())
            {
                        ...
                        buttonSize.y), "U-TURN"))
                {
                    ...
                    if (m_nextNode.IsRoundabout())
                    {
                        ...
                        {
                            if (!node && m_nextNode.m_connectedChangeLaneNodes[cIndex].m_roundaboutExit ==
                                TrafficSystemNode.RoundaboutExit.EXIT_4)
                            {
                                node = m_nextNode.m_connectedChangeLaneNodes[cIndex];
                                break;
                            }
        ...
        else if (!ChangeLaneActivated && m_nextNode.m_roadType == TrafficSystem.RoadType.LANES_MULTI &&
                 m_nextNode.m_connectedNodes.Count > 1)
        {
            int buttonCount = 2;
            float initScreenPosX = (Screen.width / 2) - ((buttonSize.x + buttonSpacerX) * (buttonCount / 2.0f));
