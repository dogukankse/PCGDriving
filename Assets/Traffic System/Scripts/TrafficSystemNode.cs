...
public class TrafficSystemNode : MonoBehaviour
{
    ...
    public TrafficSystem.RoadType
        m_roadType =
            TrafficSystem.RoadType.LANES_2; // set to normal if you are not sure. The names should explain the others.

    public bool
        m_isPrimary =
            true; // primary nodes are the nodes that the anchor traffic system piece will look to make a connection / link to forming the global path

    public int
        m_lane = 0; // this is handy when there is more than 1 lane for an object to follow. This will make sure the object stays in the correct lane, unless random lane changing is greater than 0.0f

    public bool
        m_onlyConnectWithSameLane = false; // if true, only nodes with the same lane number can connect to this one

    public TrafficSystem.DriveSide
        m_driveSide = TrafficSystem.DriveSide.LEFT; // which side of the road this node belongs too

    public List<TrafficSystemNode> m_connectedNodes; // holds all the possible nodes to traverse too

    public TrafficSystemNode
        m_connectedLocalNode; // holds the node to traverse between a primary and the next primary node
    ...
    public DirectionListing
        m_directionListing =
            DirectionListing.NONE; // this is used to indicate if a node has multiple directions that a vehicle can take

    public RoundaboutExit
        m_roundaboutExit =
            RoundaboutExit.NONE; // this is used to indicate what roundabout exit a change lane node is attached too
    ...
    void Awake()
    {
        ...
        GameObject obj =
            TrafficSystemGameUtils.FindParentItem(gameObject,
                TrafficSystemGameUtils.GameObjectItem.TRAFFIC_SYSTEM_PIECE);
...
//	}
    ...
    public TrafficSystemNode GetNextNode(TrafficSystemVehicle a_vehicle, bool a_checkLocalConnectedNode = true,
        List<TrafficSystemNode> a_blockedNodes = null)
    {
                    ...
                    if (node)
                    {
                        ...
                        if (!blockNode)
                        {
                            if (node.m_lane != m_lane ||
                                (node.m_lane == m_lane && node.m_roadType == TrafficSystem.RoadType.OFFRAMP))
                                return node;
                            else
                                sameLaneNode = node;
                        }
    ...
    public bool IsRoundabout()
    {
        ...
        {
            if (Parent.transform.parent.GetComponent<TrafficSystemPiece>().m_roadPieceType ==
                TrafficSystemPiece.RoadPieceType.ROUNDABOUT)
                return true;
        }
...
//					Gizmos.DrawCube( (transform.position - (dir.normalized * ((dir.magnitude / 2) + scaleFactorSphere))) + offset, new Vector3(scaleFactorCube * 1.4f, scaleFactorCube * 1.4f, scaleFactorCube * 1.4f) );
                    ...
                    Gizmos.DrawCube(
                        (transform.position - (dir.normalized * ((dir.magnitude / 2) + scaleFactorSphere))) + offset,
                        new Vector3(scaleFactorCube, scaleFactorCube, scaleFactorCube));
                    Gizmos.color = Color.white;
                    Gizmos.DrawSphere((transform.position - (dir.normalized * (dir.magnitude / 2))) + offset,
                        scaleFactorSphere);
                }
            }
        }
        ...
        for (int cIndex = 0; cIndex < m_connectedChangeLaneNodes.Count; cIndex++)
        {
            if (m_connectedChangeLaneNodes[cIndex]) // && m_connectedChangeLaneNodes[cIndex].m_connectedNodes.Count > 0)
            {
                ...
                Gizmos.DrawLine(transform.position + offset,
                    m_connectedChangeLaneNodes[cIndex].transform.position + offset);
