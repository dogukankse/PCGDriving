...
public class TrafficSystem : MonoBehaviour
{
    ...
    public RoadQuality GetRoadQualityVal()
    {
        return m_roadQualityVal;
    }
    ...
    public int
        m_vehicleSpawnCountMax =
            -1; // if -1 then unlimited vehicles can spawn. If higher than only this amount will ever spawn using the Traffic System spawn options

    public bool m_randomVehicleSpawnPerNode = false;
    [Range(0, 1)] public float m_randomVehicleSpawnChancePerNode = 0.0f;
    public float m_randVehicleVelocityMin = 1.0f;
    public float m_randVehicleVelocityMax = 5.0f;

    public int
        m_globalSpeedLimit =
            5; // used for when individual Road Piece have a speed limit of -1 (which is the default), they will use this global limit.

    [Range(0.0f, 5.0f)]
    public float
        m_globalSpeedVariation = 1.5f; // used to generate a slight variation of speed each node a vehicle gets to

    [Range(0.0f, 0.4f)]
    public float
        m_globalLanePosVariation = 0.0f; // used to generate a slight variation of lane position for all vehicles

    //	public  List<TrafficSystemVehicle> m_vehicleToSpawn      = new List<TrafficSystemVehicle>();
//	public  float               m_randVelocityMin            = 1.0f;
//	public  float               m_randVelocityMax            = 5.0f;
//
//	[Range(0.0f, 1.0f)]
//	public  float               m_chanceToSpawnOnNode        = 0.1f;
    ...
    public List<TrafficSystemVehicle> GetSpawnedVehicles()
    {
        return m_spawnedVehicles;
    }

    [HideInInspector] public bool m_swapAnchorDimensions = false;
    [HideInInspector] public bool m_swapEditDimensions = false;
    [HideInInspector] public bool m_swapOffsetDir = false;
    [HideInInspector] public bool m_swapOffsetSize = false;
    [HideInInspector] public bool m_negateOffsetSize = false;
    ...
    public void PositionTooltip(TrafficSystemTooltip a_tooltip, TrafficSystemPiece a_obj)
    {
        switch (a_tooltip)
        {
            case TrafficSystemTooltip.ANCHOR:
            {
                if (TooltipAnchor)
                {
                    TooltipAnchor.transform.position = new Vector3(a_obj.transform.position.x,
                        a_obj.transform.position.y + a_obj.m_renderer.bounds.extents.y + 2.0f,
                        a_obj.transform.position.z);
                }
            }
                break;
            case TrafficSystemTooltip.EDIT:
            {
                if (TooltipEdit)
                {
                    TooltipEdit.transform.position = new Vector3(a_obj.transform.position.x,
                        a_obj.transform.position.y + a_obj.m_renderer.bounds.extents.y + 2.4f,
                        a_obj.transform.position.z);
                }
            }
                break;
    ...
    void OnDrawGizmos()
    {
        ...
        if (CLRevealObjectsFrom.Count > 0 && CLRevealObjectsFrom.Count == CLRevealObjectsTo.Count)
        {
            ...
            for (int rIndex = 0; rIndex < CLRevealObjectsFrom.Count; rIndex++)
            {
                ...
                Gizmos.DrawLine(CLRevealObjectsFrom[rIndex].position + offset,
                    CLRevealObjectsTo[rIndex].position + offset);

                Vector3 dir = CLRevealObjectsFrom[rIndex].position - CLRevealObjectsTo[rIndex].position;
                Gizmos.color = Color.yellow;
                Gizmos.DrawCube(
                    (CLRevealObjectsFrom[rIndex].position -
                     (dir.normalized * ((dir.magnitude / 2) + scaleFactorSphere))) + offset,
                    new Vector3(scaleFactorCube, scaleFactorCube, scaleFactorCube));
                Gizmos.color = Color.red;
                Gizmos.DrawSphere(
                    (CLRevealObjectsFrom[rIndex].position - (dir.normalized * (dir.magnitude / 2))) + offset,
                    scaleFactorSphere);
