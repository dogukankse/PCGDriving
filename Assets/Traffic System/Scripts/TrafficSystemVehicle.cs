...
public class TrafficSystemVehicle : MonoBehaviour
{
    ...
    public TrafficSystemNode
        m_nodeToPathTo =
            null; // if this is set then the vehicle will try to get to this node via the shortest possible route

    private List<TrafficSystemNode>
        m_nodeToPathToRoute =
    ...
    public float
        m_nextNodeThreshold =
            1.0f; // determines how far away we need to be from the target node we are going to before we can change nodes.
    ...
    public bool
        m_useRoadSpeedLimits =
            false; // enable this if you want the vehicle to use the TrafficSystem.cs m_globalSpeedLimit and individual TrafficSystemPiece.cs m_speedLimit instead of this vehicles m_velocityMax
    ...
    public bool
        m_changeMaxVelocityOnSpeedLimitInc =
            true; // set this to false if you want the vehicle to ignore the road speed so it doesn't increase the vehicles max speed initially set... meaning the car will never go beyond the initial max speed even if the road says it can go faster in a certain area.
...
//	[Range(0.0f, 0.15f)]
//	public  float                       m_randomVelocityOffsetMin = 0.0f;                // this is used to give a small difference of velocity between each individually spawned vehicle even if they have the same speed. This is the min possible value (set to zero for no offset)
//	[Range(0.0f, 0.15f)]
//	public  float                       m_randomVelocityOffsetMax = 0.0f;                // this is used to give a small difference of velocity between each individually spawned vehicle even if they have the same speed. This is the max possible value (set to zero for no offset)
//	private float                       m_randomVelocityOffset    = 0.0f;                // this is used to give a small difference of velocity between each individually spawned vehicle even if they have the same speed. This is the store result of a randomised min / max on awake, set both min and max to zero to for no offset.

    [Range(0.0f, 1.0f)]
    public float
        m_randomLaneChange =
            0.2f; // 0.0f = don't change lanes at all, 1.0f change lanes (if possible) each time we visit a new node

    [Range(0.0f, 1.0f)]
    public float
        m_randomChanceToOvertake =
            0.5f; // 0.0f = don't change lanes at all, 1.0f change lanes (if possible) each time we identify there is a vehicle in front of us and we are on a highway

    [Range(0.0f, 1.0f)]
    public float
        m_chanceOfUsingOfframp =
            0.2f; // 0.0f = don't use offramps ever, 1.0f always use offramps (if possible) each time we visit a new node

    [Range(0.0f, 1.0f)]
    public float
        m_chanceOfDirChange =
            0.2f; // 0.0f = don't change direction, 1.0f always change direction (if possible) each time we visit a new node

    public Rigidbody
        m_rigidbody = null; // if set, this is how we will move. If not set, we will use the gameObjects position.

    public Collider
        m_collider =
            null; // assign the main body collider that will sit above the wheels so pathing works correctly without any collision issues with the road. 

    public Collider
        m_colliderOnCrash =
            null; // assign the main body collider that will consume the entire vehicle so that the wheels don't go through the world when the rigidbody gravity is switched on. 
    ...
    [Range(0.0f, 100.0f)] public float m_vehicleCheckDistRay = 14.0f;
    [Range(0.0f, 20.0f)] public float m_vehicleCheckDistLeftRayOffset = 1.0f;
    [Range(0.0f, 20.0f)] public float m_vehicleCheckDistRightRayOffset = 1.0f;
    [Range(0.0f, 1.0f)] public float m_vehicleCheckDistRayMimicSpeedThreshold = 0.4f;

    public bool
        m_vehicleCheckOvertakeOnHighway =
            true; // turn this check off if you don't want the vehicle to check to see if it can get into a faster lane.

    public float
        m_vehicleCheckDistOvertakeHighwayOnly =
            3.2f; // set this well ahead of your vehicle so it knows that when this sphere hits a vehicle in front it should check to change to a faster lane!
    ...
    public Vector3
        m_vehicleCheckOffset =
            Vector3.zero; // this offsets all vehicle check values in case the pivot point for a vehicle is incorrect.
    ...
    public float
        m_trafficLightCoolDownMax =
            3.0f; // once we go through a set of traffic lights this determines a cool down peroid before we stop at another set of traffic lights ... it is used for allowing bigger vehicles a cool down peroid to move forward without stopping at adjacent traffic lights of the same intersection
    ...
    public bool m_killOnEmptyPath = false;
...
//		m_randomVelocityOffset = Random.Range(m_randomVelocityOffsetMin, m_randomVelocityOffsetMax);
    ...
    public virtual void Start()
    {
        if (TrafficSystem.Instance)
        {
            ...
            m_lanePosVariation = Random.Range(-TrafficSystem.Instance.m_globalLanePosVariation,
                TrafficSystem.Instance.m_globalLanePosVariation);
        }
...
//						transform.position = nodeSearch.transform.position;
            ...
            for (int nIndex = 0; nIndex < currentNodeToSearch.m_connectedChangeLaneNodes.Count; nIndex++)
            {
                ...
                for (int uIndex = 0; uIndex < m_nodeToPathToAllNodesEverUsed.Count; uIndex++)
                {
                    if (m_nodeToPathToAllNodesEverUsed[uIndex] ==
                        currentNodeToSearch.m_connectedChangeLaneNodes[nIndex])
                        alreadyUsed = true;
                }

                if (!alreadyUsed)
                {
                    float dist = (m_nodeToPathTo.transform.position -
                                  currentNodeToSearch.m_connectedChangeLaneNodes[nIndex].transform.position).magnitude;
            ...
            for (int nIndex = 0; nIndex < currentNodeToSearch.m_connectedNodes.Count; nIndex++)
            {
                ...
                {
                    float dist = (m_nodeToPathTo.transform.position -
                                  currentNodeToSearch.m_connectedNodes[nIndex].transform.position).magnitude;
...
//				transform.position = nodeSearch.transform.position;
            ...
            else
            {
                ...
                for (int aIndex = 0; aIndex < m_nodeToPathToAllNodesEverUsed.Count; aIndex++)
                {
                    ...
                    for (int nIndex = 0; nIndex < nodeSearchTmp.m_connectedNodes.Count; nIndex++)
                    {
                        ...
                        {
                            float dist = (m_nodeToPathTo.transform.position -
                                          nodeSearchTmp.m_connectedNodes[nIndex].transform.position).magnitude;
                    ...
                    for (int nIndex = 0; nIndex < nodeSearchTmp.m_connectedNodes.Count; nIndex++)
                    {
                        ...
                        else
                        {
                            if (nodeSearchTmp.m_connectedLocalNode)
                            {
                                ...
                                {
                                    nodeSearch = nodeSearchTmp.m_connectedLocalNode;
                                    distToDest = (m_nodeToPathTo.transform.position - nodeSearch.transform.position)
                                        .magnitude;
                                    m_nodeToPathToRoute.Add(nodeSearchTmp.m_connectedLocalNode);
                                    m_nodeToPathToAllNodesEverUsed.Add(nodeSearch);
                                    break;
                            ...
                            for (int cLIndex = 0;
                                cLIndex < nodeSearchTmp.m_connectedNodes[nIndex].m_connectedChangeLaneNodes.Count;
                                cLIndex++)
                            {
                                if (nodeSearch)
                                    break;
                                ...
                                {
                                    if (m_nodeToPathToAllNodesEverUsed[uIndex] == nodeSearchTmp.m_connectedNodes[nIndex]
                                        .m_connectedChangeLaneNodes[cLIndex])
                                        alreadyUsed = true;
                                }

                                if (!alreadyUsed)
                                {
                                    float dist = (m_nodeToPathTo.transform.position - nodeSearchTmp
                                        .m_connectedNodes[nIndex].m_connectedChangeLaneNodes[cLIndex]
                                        .transform.position).magnitude;

                                    nodeSearch = nodeSearchTmp.m_connectedNodes[nIndex]
                                        .m_connectedChangeLaneNodes[cLIndex];
                                    distToDest = dist;
                                    m_nodeToPathToRoute.Add(nodeSearchTmp.m_connectedNodes[nIndex]
                                        .m_connectedChangeLaneNodes[cLIndex]);
                                    m_nodeToPathToAllNodesEverUsed.Add(nodeSearch);
                                    break;
                                }
        ...
        StartCoroutine(TestTraverse());
        m_calculatingPathToTake = false;
    }

    IEnumerator TestTraverse()
    {
    ...
    IEnumerator ProcessBlinkers()
    {
        while (true)
        {
            if (IsTurningLeft())
            {
                ...
                {
                    m_lightsBlinkerLeft[rIndex].gameObject
                        .SetActive(!m_lightsBlinkerLeft[rIndex].gameObject.activeSelf);
                }
            ...
            else if (IsTurningRight())
            {
                ...
                {
                    m_lightsBlinkerRight[rIndex].gameObject
                        .SetActive(!m_lightsBlinkerRight[rIndex].gameObject.activeSelf);
                }
...
//		CanSeeVehicleViaSphere();
        ...
        else if (IsStopped() && m_enableWaitingDestroyTimer)
        {
            ...
            if (m_waitingDestroyTimer >= m_waitingDestroyTimerMax)
            {
                if (Debug.isDebugBuild)
                    Debug.LogWarning(
                        "Vehicle Destroyed - m_waitingDestroyTimerMax - (m_enableWaitingDestroyTimer = true) - Vehicle was waiting too long in the one position");
        ...
        if (m_nextNode)
        {
            ...
            if (isOnHighway)
            {
                RaycastHit hitInfo;
                VehicleHit = null;
                if (Physics.SphereCast((transform.position + m_vehicleCheckOffset),
                    m_vehicleCheckRadiusOvertakeHighwayOnly, transform.forward, out hitInfo,
                    m_vehicleCheckDistOvertakeHighwayOnly))
                {
                    ...
                    if (vehicle)
                    {
                        if (vehicle != this && vehicle.VehicleHit != this &&
                            vehicle != m_previousVehicleToTryOvertaking)
                        {
                ...
                RaycastHit[] hitInfoAll = Physics.SphereCastAll((transform.position + m_vehicleCheckOffset),
                    m_vehicleCheckRadius, transform.forward, m_vehicleCheckDist);
                VehicleHit = null;
            ...
            {
                RaycastHit[] hitInfo = Physics.SphereCastAll((transform.position + m_vehicleCheckOffset),
                    (m_vehicleCheckRadius * m_vehicleCheckRadiusMid), transform.forward,
                    m_vehicleCheckDist - m_vehicleCheckDistMid);
        ...
        if (!StopMoving && TrafficLight && !CrashDetected
        ) // determine if there is there is room to move throught the intersection
        {
...
//						velocityRatio = VehicleInSights.m_velocity / m_velocity;
            ...
            if (Accelerate) // are we accelerating?
                m_velocity =
                    m_velocity +
                    ((m_accelerationRate * ratio) *
                     Time.deltaTime); // add to the current velocity according while accelerating
            else
                m_velocity =
                    m_velocity -
                    ((m_decelerationRate * ratio) *
                     Time.deltaTime); // subtract from the current velocity while decelerating
            ...
            if (VehicleHit)
                ...
                m_velocity =
                    Mathf.Clamp(m_velocity, m_velocityMin,
                        m_velocityMax); // ensure the velocity never goes out of the min/max boundaries
            ...
            if (m_rigidbody) // if we have a rigidbody, use the following code to move us
            {
                ...
                transform.forward =
                    Vector3.Lerp(transform.forward, dir.normalized,
                        rotSpeed * Time
                            .deltaTime); // rotate our forward directoin over time to face the node we are moving towards
            }
            else // no rigidbody then use the following code to move us
            {
                if (m_collider || GetComponent<Collider>()
                ) // it generally is a bad idea to move something with a collider, so we should tell someone about it if this is happening. See Unity Docs for more info: http://docs.unity3d.com/ScriptReference/Collider.html
                    Debug.LogWarning(
                        "Traffic System Warning -> VehicleBase has a collider. You should think about moving the object with a rigidbody instead.");
            ...
            if (dir.magnitude < m_nextNodeThreshold
            ) // if we are close enough to the node we were travelling towards then check for the next one
            {
...
//					}
//
                SetNextNode(m_nextNode.GetNextNode(this,
                    true /*, blockedNodes*/)); // gets the next required node to go to and start the process all over again
                //}
            }
        }
...
//				Debug.Log( "GO!");
            ...
            else if (TrafficLight.m_intersection.m_intersectionSystem ==
                     TrafficSystemIntersection.IntersectionSystem.DUAL)
            {
...
//					Debug.Log( "GO!");
            ...
            Debug.LogWarning(
                "Traffic System Traffic Light is missing a parent intersection! See the example traffic system intersection and the structure setup as this is what you need to mimic");
...
//			Debug.Log("Traffic light setted");
    ...
    protected void SetNextNode(TrafficSystemNode a_node)
    {
        ...
        {
            Debug.LogWarning(
                "Vehicle node invalid, check all nodes on this road piece have correct links as this is probably the issue!: \""
    ...
    {
        m_velocity =
            0.0f; //m_velocity - (m_decelerationRate * Time.deltaTime);        // subtract from the current velocity while decelerating
    ...
    public bool CanFitAcrossIntersection()
    {
        ...
        RaycastHit[] rays = Physics.RaycastAll((transform.position + m_vehicleCheckOffset), transform.forward,
            m_intersectionCheckDistRay);
    ...
    public TrafficSystemVehicle CanSeeVehicleViaRay(bool a_checkVehicleIsMoving = false)
    {
        ...
        {
            if (Physics.Raycast(
                ((transform.position + m_vehicleCheckOffset) + (transform.right * m_vehicleCheckDistRightRayOffset)),
                transform.forward, out hitInfo, m_vehicleCheckDistRay))
            {
                TrafficSystemVehicle vehicle = hitInfo.transform.GetComponent<TrafficSystemVehicle>();
                if (vehicle && vehicle != this && VehicleIsOnSameSide(vehicle))
    ...
    public bool CanSeeVehicleViaSphere(bool a_checkVehicleIsMoving = false)
    {
        ...
        hits = Physics.CapsuleCastAll(p1, p2, m_vehicleCheckRadiusAtLightsCapsule, transform.forward,
            m_vehicleCheckDistAtLightsCapsule);
    ...
    public bool IsTrafficJamOnUpcomingNodes()
    {
        if (!m_nextNode)
            return false;

        TrafficSystemVehicle vehicle = null;
        TrafficSystemNode nodeToCheck = null;

        bool trafficJam = false;
        /*
        if(m_nextNode.m_connectedLocalNode)
        {
            // check first level
            nodeToCheck = m_nextNode.m_connectedLocalNode;
            vehicle     = VehicleExistsOnNode(nodeToCheck);
            ...
            if(!trafficJam)
            {
                for(int nIndex = 0; nIndex < nodeToCheck.m_connectedNodes.Count; nIndex++)
                {
                    TrafficSystemNode node = nodeToCheck.m_connectedNodes[nIndex];
                    if(VehicleExistsOnNode(node))
                    {
                        trafficJam = true;
                        break;
                    }
            ...
            // check second level
            if(!trafficJam && nodeToCheck && nodeToCheck.m_connectedLocalNode)
            {
                nodeToCheck = nodeToCheck.m_connectedLocalNode;
                vehicle     = VehicleExistsOnNode(nodeToCheck);
        ...
        else if(m_nextNode.m_connectedChangeLaneNodes.Count > 0)
        {
            // check first level
            nodeToCheck = m_nextNode.m_connectedChangeLaneNode;
            vehicle     = VehicleExistsOnNode(nodeToCheck);
            ...
            // check second level
            if(!trafficJam && nodeToCheck && nodeToCheck.m_connectedChangeLaneNode)
            {
                nodeToCheck = nodeToCheck.m_connectedChangeLaneNode;
                vehicle     = VehicleExistsOnNode(nodeToCheck);
    ...
    public bool IsTurningRight()
    {
        ...
        if (m_nextNode.m_roadType == TrafficSystem.RoadType.CHANGE_LANE &&
            m_nextNode.m_driveSide == TrafficSystem.DriveSide.RIGHT)
            return true;
    ...
    public bool IsTurningLeft()
    {
        ...
        if (m_nextNode.m_roadType == TrafficSystem.RoadType.CHANGE_LANE &&
            m_nextNode.m_driveSide == TrafficSystem.DriveSide.LEFT)
            return true;
...
//			FindNodeDistanceCheckPosY();
        ...
        Gizmos.DrawWireSphere((transform.position + m_vehicleCheckOffset) + (transform.forward * m_vehicleCheckDist),
            m_vehicleCheckRadius);

        if (m_vehicleCheckOvertakeOnHighway)
        {
            Gizmos.color = Color.magenta;
            Gizmos.DrawWireSphere(
                (transform.position + m_vehicleCheckOffset) +
                (transform.forward * m_vehicleCheckDistOvertakeHighwayOnly), m_vehicleCheckRadiusOvertakeHighwayOnly);
        }
        ...
        if (m_enableVehicleCheckMid)
        {
            ...
            Gizmos.DrawWireSphere(
                (transform.position + m_vehicleCheckOffset) +
                (transform.forward * (m_vehicleCheckDist - m_vehicleCheckDistMid)),
                (m_vehicleCheckRadius * m_vehicleCheckRadiusMid));
        }

        Gizmos.color = Color.yellow;
        Gizmos.DrawLine((transform.position + m_vehicleCheckOffset),
            (transform.position + m_vehicleCheckOffset) + (transform.forward * m_intersectionCheckDistRay));

        Gizmos.color = Color.red;
        Gizmos.DrawLine(
            ((transform.position + m_vehicleCheckOffset) + (-transform.right * m_vehicleCheckDistLeftRayOffset)),
            ((transform.position + m_vehicleCheckOffset) + (-transform.right * m_vehicleCheckDistLeftRayOffset)) +
            (transform.forward * m_vehicleCheckDistRay));
        Gizmos.DrawLine(
            ((transform.position + m_vehicleCheckOffset) + (transform.right * m_vehicleCheckDistRightRayOffset)),
            ((transform.position + m_vehicleCheckOffset) + (transform.right * m_vehicleCheckDistRightRayOffset)) +
            (transform.forward * m_vehicleCheckDistRay));
...
//	void ProcessSpline()
//	{
//		if(m_traversePath && m_nextNode.GetPath( this ) != null)
//		{
//			iTween.MoveTo( gameObject, iTween.Hash( "path", m_nextNode.GetPath( this ), 
//			                                       "speed", m_velocity, 
//			                                       "orienttopath", m_orientToPath, 
//			                                       "looktime", 1.5f,//m_rotationSpeed, 
//			                                       "easetype", m_easeType,
//			                                       "lookahead", 1.0f,
//			                                       "looptype", m_loopType,
...
//		m_nextNode = m_nextNode.GetNextNode( this, false );
//	}
}