...
public class TrafficSystemIntersection : MonoBehaviour
{
    ...
    {
        SINGLE =
            0, // SINGLE = one light will turn green at any one time and cycle through each light in the m_lights container.

        DUAL =
            1 // DUAL   = all the lights in m_lightsA will turn green and then all the lights in m_lightsB will turn green allow for multi lane movement within an intersection
    }

    public bool
        m_automatic =
            true; // This allows users to toggle the traffic intersection (lights) manually if they choose too. Auto is default so they don't have to worry about it, just place in the editor and away they go.


    public IntersectionSystem
        m_intersectionSystem =
            IntersectionSystem
                .SINGLE; // SINGLE = one light will turn green at any one time and cycle through each light in the m_lights container.
    ...
    public List<TrafficSystemTrafficLight>
        m_lights =
            new List<TrafficSystemTrafficLight>(); // these are found automatically on start. You can however put them in manually if you really want too but you should comment out Refresh() in the Awake() section if that is the case.

    public List<TrafficSystemTrafficLight>
        m_lightsA =
            new List<TrafficSystemTrafficLight>(); // used only with DUAL Intersection System... see above comments on m_interactiveSystem
    ...
    private bool
        m_lightsAGreen =
            true; // used only with DUAL Intersection System... This is a toggle to know when to turn all the m_lightsA or m_lightsB green
    ...
    public float
        m_yellowDuration = 2.0f; // FYI, the green duration is on an individual light (TrafficSystemTrafficLight.cs)

    public float m_redDuration = 2.0f;

    [Range(0.0f, 1.0f)]
    public bool
        m_ignoreCanFitAcrossIntersectionCheck =
            true; // If you want vehicles to stop and check that the intersection is clear, turn this on... will help prevent vehicles getting stuck in the middle of the intersection

    public List<PedestrianNode>
        m_pedestrianNodes =
            new List<PedestrianNode>(); // if nodes are added then it will block all pedestrians from walking across the road when the lights are green or yellow.

    public float m_walkDuration = 6.0f; // the time pedestrians have to walk across the road

    public float
        m_dontWalkPercentage =
            0.40f; // if there is a pedestrian node linked to the TrafficSystemTrafficLight object then it will use the percentage of teh red duration to determine when to walk.

    public int
        m_walkSequenceMax =
            4; // the amount of light change durations must happen before the pedestrians will be allowed to walk across the road   
