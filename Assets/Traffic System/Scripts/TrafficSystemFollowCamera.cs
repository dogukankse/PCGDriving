...
public class TrafficSystemFollowCamera : MonoBehaviour
{
    ...
    [Range(0.0f, 10.0f)] public float m_vehicleCameraFaceForwardDelay = 2.0f;
    ...
    void Start()
    {
        ...
        {
            m_vehicleCamera.transform.position = m_vehicleToFollow.transform.position -
                (m_vehicleToFollow.transform.forward.normalized *
                 m_vehicleCameraOffsetBehind) + new Vector3(0.0f,
                    m_vehicleCameraOffsetAbove, 0.0f);
    ...
    void Update()
    {
        if (Application.isPlaying && m_enableVehicleCamera)
        {
            ...
            {
                Ray ray = m_vehicleCamera.ScreenPointToRay(new Vector3(Input.mousePosition.x, Input.mousePosition.y,
                    0.0f));
            ...
            if (m_vehicleCamera && m_vehicleToFollow)
            {
                m_vehicleCamera.transform.position = m_vehicleToFollow.transform.position -
                    (m_vehicleToFollow.transform.forward.normalized *
                     m_vehicleCameraOffsetBehind) + new Vector3(0.0f,
                        m_vehicleCameraOffsetAbove, 0.0f);
                m_vehicleCamera.transform.forward = Vector3.Slerp(m_vehicleCamera.transform.forward,
                    m_vehicleToFollow.transform.forward + m_vehicleCameraRotation,
                    m_vehicleCameraFaceForwardDelay * Time.deltaTime);
            }
        }
    }
