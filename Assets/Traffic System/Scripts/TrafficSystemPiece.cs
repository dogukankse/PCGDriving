...
public class TrafficSystemPiece : MonoBehaviour
{
    ...
    [Range(-1, 20)] public int m_speedLimit = -1;
    ...
    public void Refresh()
    {
        ...
        {
            GameObject obj =
                TrafficSystemGameUtils.FindItem(gameObject, TrafficSystemGameUtils.GameObjectItem.RENDERER);
        ...
        for (int lIndex = 0; lIndex < m_primaryLeftLaneNodes.Count; lIndex++)
        {
            ...
            for (int cIndex = 0; cIndex < cleanList.Count; cIndex++)
            {
                ...
                if (node)
                {
                    bool found = false;
                    for (int nIndex = 0;
                        nIndex < m_primaryLeftLaneNodes[lIndex].m_connectedChangeLaneNodes.Count;
                        nIndex++)
                    {
        ...
        for (int lIndex = 0; lIndex < m_primaryRightLaneNodes.Count; lIndex++)
        {
            ...
            for (int cIndex = 0; cIndex < cleanList.Count; cIndex++)
            {
                ...
                if (node)
                {
                    bool found = false;
                    for (int nIndex = 0;
                        nIndex < m_primaryRightLaneNodes[lIndex].m_connectedChangeLaneNodes.Count;
                        nIndex++)
                    {
                        TrafficSystemNode checkNode =
                            m_primaryRightLaneNodes[lIndex].m_connectedChangeLaneNodes[nIndex];
        ...
        for (int lIndex = 0; lIndex < m_secondaryLeftLaneNodes.Count; lIndex++)
        {
            ...
            for (int cIndex = 0; cIndex < cleanList.Count; cIndex++)
            {
                ...
                if (node)
                {
                    bool found = false;
                    for (int nIndex = 0;
                        nIndex < m_secondaryLeftLaneNodes[lIndex].m_connectedChangeLaneNodes.Count;
                        nIndex++)
                    {
                        TrafficSystemNode checkNode =
                            m_secondaryLeftLaneNodes[lIndex].m_connectedChangeLaneNodes[nIndex];
        ...
        for (int lIndex = 0; lIndex < m_secondaryRightLaneNodes.Count; lIndex++)
        {
            ...
            for (int cIndex = 0; cIndex < cleanList.Count; cIndex++)
            {
                ...
                if (node)
                {
                    bool found = false;
                    for (int nIndex = 0;
                        nIndex < m_secondaryRightLaneNodes[lIndex].m_connectedChangeLaneNodes.Count;
                        nIndex++)
                    {
                        TrafficSystemNode checkNode =
                            m_secondaryRightLaneNodes[lIndex].m_connectedChangeLaneNodes[nIndex];
        ...
        for (int lIndex = 0; lIndex < m_leftLaneChangeNodes.Count; lIndex++)
        {
            ...
            for (int cIndex = 0; cIndex < cleanList.Count; cIndex++)
            {
                ...
                if (node)
                {
                    bool found = false;
                    for (int nIndex = 0;
                        nIndex < m_leftLaneChangeNodes[lIndex].m_connectedChangeLaneNodes.Count;
                        nIndex++)
                    {
        ...
        for (int lIndex = 0; lIndex < m_rightLaneChangeNodes.Count; lIndex++)
        {
            ...
            for (int cIndex = 0; cIndex < cleanList.Count; cIndex++)
            {
                ...
                if (node)
                {
                    bool found = false;
                    for (int nIndex = 0;
                        nIndex < m_rightLaneChangeNodes[lIndex].m_connectedChangeLaneNodes.Count;
                        nIndex++)
                    {
...
//			return new Bounds( transform.position, new Vector3(0.0f, 0.0f, 0.0f) );
    ...
    public void SpawnRandomVehicle(TrafficSystemVehicle a_vehiclePrefab, TrafficSystemNode node)
    {
        ...
        if (node)
        {
            ...
            if (node.m_driveSide == TrafficSystem.DriveSide.LEFT)
            {
                rotation.eulerAngles = new Vector3(rotation.eulerAngles.x, 180 - rotation.eulerAngles.y,
                    rotation.eulerAngles.z);
            }

            TrafficSystemVehicle vehicle =
                Instantiate(a_vehiclePrefab, node.transform.position, rotation) as TrafficSystemVehicle;
            vehicle.m_nextNode = node;
            vehicle.m_velocityMax = Random.Range(TrafficSystem.Instance.m_randVehicleVelocityMin,
                TrafficSystem.Instance.m_randVehicleVelocityMax);
...
//			Gizmos.DrawCube( transform.position, rendererSize );
    ...
    public bool IsRendererCustom()
    {
        ...
        for (int pIndex = 0; pIndex < m_roadModelsOriginal.Length; pIndex++)
        {
            if (m_roadModelsOriginal[pIndex].GetComponent<Renderer>() &&
                m_roadModelsOriginal[pIndex].GetComponent<Renderer>() == m_renderer)
            {
                found = false;
                break;
            }
        ...
        if (!found)
        {
            for (int pIndex = 0; pIndex < m_roadModelsVersion2.Length; pIndex++)
            {
                if (m_roadModelsVersion2[pIndex].GetComponent<Renderer>() &&
                    m_roadModelsVersion2[pIndex].GetComponent<Renderer>() == m_renderer)
                {
                    found = false;
                    break;
                }
            ...
            if (!found)
            {
                for (int pIndex = 0; pIndex < m_roadModelsVersion2Mobile.Length; pIndex++)
                {
                    if (m_roadModelsVersion2Mobile[pIndex].GetComponent<Renderer>() &&
                        m_roadModelsVersion2Mobile[pIndex].GetComponent<Renderer>() == m_renderer)
                    {
                        found = false;
                        break;
                    }
