...
public class TwistRecognizerInspector : GestureRecognizerInspector<TwistRecognizer>
{
    ...
    protected static GUIContent LABEL_MinRotation = new GUIContent("Minimum Rotation",
        "Minimum amount of rotation required to start the rotation gesture (in degrees)");

    protected static GUIContent LABEL_AllowPivotAroundFinger = new GUIContent("Allow Pivot Around First Finger",
        "Allow the user to perform a twist gesture by rotating the second finger around the first one (which must stay motionless)");
    ...
    protected static GUIContent LABEL_PivotMoveTolerance = new GUIContent("Pivot Movement Tolerance",
        "How far the pivot finger can move from its initial position without causing the gesture to fail");
