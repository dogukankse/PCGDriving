...
public class PointCloudRecognizerInspector : GestureRecognizerInspector<PointCloudRegognizer>
{
    protected static GUIContent LABEL_Templates = new GUIContent("Gesture Templates List",
        "List of gesture templates that will be matched against the user's gesture");

    protected static GUIContent LABEL_MinDistanceBetweenSamples = new GUIContent("Sampling Distance",
        "Minimum distance between two consecutive finger position samples. Smaller means more accurate recording of the gesture, but more samples to process.");

    protected static GUIContent LABEL_MaxMatchDistance = new GUIContent("Max Match Distance",
        "Threshold value that controls how accurate the user-generated gesture must be in order to match its corresponding template gesture. The lower the value, the more accurate the user must be.");
    ...
    protected override void OnSettingsUI()
    {
        ...
        Gesture.MinDistanceBetweenSamples =
            EditorGUILayout.FloatField(LABEL_MinDistanceBetweenSamples, Gesture.MinDistanceBetweenSamples);
