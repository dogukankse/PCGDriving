...
{
    protected static GUIContent LABEL_ResetMode = new GUIContent("Reset Mode",
        "Controls when the gesture should reset its internal state after a successful or failed recognition");

    protected static GUIContent LABEL_RequiredFingerCount = new GUIContent("Required Finger Count",
        "Exact number of fingers necessary to perform this gesture");

    protected static GUIContent LABEL_MaxSimultaneousGestures = new GUIContent("Max Simultaneous Gestures",
        "Maximum number of simultaneous gestures this recognizer can track at once");

    protected static GUIContent LABEL_Exclusive = new GUIContent("Exclusive",
        "Exclusive gestures will only trigger when the current number of touches on the device matches exactly the required finger count specified above");

    protected static GUIContent LABEL_EventMessageSectionTitle = new GUIContent("Event Message Broadcasting",
        "Configure the broadcasting of gesture event via Unity's SendMessage() API");

    protected static GUIContent LABEL_EventMessageName = new GUIContent("Message Name",
        "Name/identifier of the method to invoke on the target object when this gesture event fires");

    protected static GUIContent LABEL_EventMessageTarget = new GUIContent("Message Target",
        "The target game object to send the gesture event message to.\nDefaults to current object if no value is provided.");

    protected static GUIContent LABEL_SendMessageToSelection = new GUIContent("Send Message To Selection",
        "Specify the selection object to also send the event message to");

    protected static GUIContent LABEL_Raycaster = new GUIContent("Raycaster",
        "ScreenRaycaster component used to pick collider-equipped scene objects.\nDefaults to first ScreenRaycaster component on current object if no value is provided.");

    protected static GUIContent LABEL_ClusterManager = new GUIContent("Cluster Manager",
        "FingerClusterManager component responsible for dynamically grouping fingers together");

    protected static GUIContent LABEL_Delegate = new GUIContent("Delegate",
        "(Advanced) GestureRecognizerDelegate component used to override specific gesture recognizer behaviours.\nDefaults to first GestureRecognizerDelegate component on current object if no value is provided.");
    ...
    protected virtual void OnNotices()
    {
        if (Gesture.RequiredFingerCount > 1 && !Gesture.SupportFingerClustering)
            EditorGUILayout.HelpBox(
                "This recognizer can only track a single multi-finger gesture at once. Simultaneous multi-finger recognition is not supported with the current configuration.",
                MessageType.Info);
    }
    ...
    protected virtual void OnSettingsUI()
    {
        ...
        if (GUI.enabled)
            Gesture.MaxSimultaneousGestures =
                EditorGUILayout.IntField(LABEL_MaxSimultaneousGestures, Gesture.MaxSimultaneousGestures);
        else
    ...
    protected virtual void OnMessagingUI()
    {
        ...
        string eventName = string.IsNullOrEmpty(Gesture.EventMessageName)
            ? Gesture.GetDefaultEventMessageName()
            : Gesture.EventMessageName;
        Gesture.EventMessageName = EditorGUILayout.TextField(LABEL_EventMessageName, eventName);
        Gesture.EventMessageTarget =
            EditorGUILayout.ObjectField(LABEL_EventMessageTarget, Gesture.EventMessageTarget, typeof(GameObject), true)
                as GameObject;
        Gesture.SendMessageToSelection =
            (GestureRecognizer.SelectionType) EditorGUILayout.EnumPopup(LABEL_SendMessageToSelection,
                Gesture.SendMessageToSelection);
        GUI.enabled = true;
    ...
    protected virtual void OnComponentsUI()
    {
        ...
        Gesture.Raycaster =
            EditorGUILayout.ObjectField(LABEL_Raycaster, Gesture.Raycaster, typeof(ScreenRaycaster), true) as
                ScreenRaycaster;
        Gesture.Delegate =
            EditorGUILayout.ObjectField(LABEL_Delegate, Gesture.Delegate, typeof(GestureRecognizerDelegate), true) as
                GestureRecognizerDelegate;

        GUI.enabled = Gesture.SupportFingerClustering;
        if (Gesture.SupportFingerClustering)
            Gesture.ClusterManager =
                EditorGUILayout.ObjectField(LABEL_ClusterManager, Gesture.ClusterManager, typeof(FingerClusterManager),
                    true) as FingerClusterManager;
        else
            EditorGUILayout.ObjectField(LABEL_ClusterManager, Gesture.ClusterManager, typeof(FingerClusterManager),
                true);
        GUI.enabled = true;
    ...
    protected virtual void OnToolbar()
    {
        ...
        {
            string value = string.Format("void {0}({1} gesture)", Gesture.EventMessageName,
                Gesture.GetGestureType().Name);
