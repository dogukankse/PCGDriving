...
{
    protected static GUIContent LABEL_MinDistance = new GUIContent("Min Distance",
        "Minimum distance the finger must travel in order to produce a valid swipe");

    protected static GUIContent LABEL_MaxDistance = new GUIContent("Max Distance",
        "Finger travel distance beyond which the swipe recognition will fail.\nSetting this to 0 disables the constraint");

    protected static GUIContent LABEL_MinVelocity = new GUIContent("Min Velocity",
        "Minimum speed the finger must maintain in order to produce a valid swipe gesture");

    protected static GUIContent LABEL_MaxDeviation = new GUIContent("Max Deviation",
        "Maximum angle that the swipe direction is allowed to deviate from its initial direction (in degrees)");
