...
{
    protected static GUIContent LABEL_FingerIndexFilter = new GUIContent("Finger Index Filter",
        "Filters out all active fingers except the one with this index. Settings this to -1 will track any active finger (default).");

    protected static GUIContent LABEL_EventMessageSectionTitle = new GUIContent("Event Message Broadcasting",
        "Configure the broadcasting of finger event via Unity's SendMessage() API");

    protected static GUIContent LABEL_EventMessageName = new GUIContent("Message Name",
        "Name/identifier of the method to invoke on the target object when this finger event fires");

    protected static GUIContent LABEL_EventMessageTarget = new GUIContent("Message Target",
        "The target game object to send the finger event message to.\nDefaults to current object if no value is provided.");

    protected static GUIContent LABEL_SendMessageToSelection = new GUIContent("Send Message To Selection",
        "Check this if you also want to forward the event to the object under the finger");
    ...
    protected virtual void OnMessagingUI()
    {
        ...
        MessageEventsGUI();
        Detector.MessageTarget =
            EditorGUILayout.ObjectField(LABEL_EventMessageTarget, Detector.MessageTarget, typeof(GameObject), true) as
                GameObject;
        Detector.SendMessageToSelection =
            EditorGUILayout.Toggle(LABEL_SendMessageToSelection, Detector.SendMessageToSelection);
        GUI.enabled = true;
    ...
    protected virtual void OnComponentsUI()
    {
        ...
        Detector.Raycaster =
            EditorGUILayout.ObjectField(LABEL_Raycaster, Detector.Raycaster, typeof(ScreenRaycaster), true) as
                ScreenRaycaster;
    }
