...
public class PointCloudRegognizer : DiscreteGestureRecognizer<PointCloudGesture>
{
    ...
    static float CloudDistance(List<Point> points1, List<Point> points2, int startIndex)
    {
        ...
        do
        {
            ...
            for (int j = 0; j < numPoints; ++j)
            {
                ...
                {
                    float distance =
                        Vector2.Distance(points1[i].Position,
                            points2[j].Position); //OPTIMIZEME: replace by square distance compare
            ...
            float weight =
                1 - ((i - startIndex + points1.Count) % points1.Count) /
                points1.Count; // [0::1] assign decreasing confidence weights to point matchings.
    ...
    protected override GestureRecognitionState OnRecognize(PointCloudGesture gesture,
