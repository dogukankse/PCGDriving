...
public class TBDragToMove : MonoBehaviour
{
    ...
    Vector3
        physxDragMove =
            Vector3.zero; // used for rigidbody drag only. This stores the drag amount to apply during the physics update in FixedUpdate()
        ...
        else if (Dragging)
        {
            ...
            if (gesture.Phase == ContinuousGesturePhase.Updated)
            {
                ...
                else
                {
                    ...
                    if (ProjectScreenPointOnDragPlane(tf.position, draggingFinger.PreviousPosition,
                            out prevFingerPos3d) &&
