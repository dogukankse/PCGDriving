...
public class PedestrianSystemEditor : Editor
{
    ...
    void Awake()
    {
        ...
        if (!PedestrianSystem)
            Debug.LogError(
                "Pedestrian System Error -> \"Pedestrian System\" script must be in the scene. It is missing. Drop in the \"PedestrianSystem\" Prefab to fix this.");
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Generate Node", GUILayout.Height(49)))
        {
            ...
            {
                Debug.LogError(
                    "Node Prefab not set. You can find one in \"Assets\\Pedestrian System\\Prefabs\\Pathing\". Node Generation Cancelled.");
                return;
            }

            PedestrianNode node =
                PrefabUtility.InstantiatePrefab(PedestrianSystem.Instance.m_nodePrefab) as PedestrianNode;
...
//		}
    ...
    void CheckForAnchorNode()
    {
        ...
        for (int cIndex = 0; cIndex < AnchorNodeFolder.childCount; cIndex++)
        {
            ...
            {
                SetPedestrianNode(PedestrianSystem.Tooltip.ANCHOR,
                    AnchorNodeFolder.GetChild(cIndex).GetComponent<PedestrianNode>());
            }
        }
    }
    ...
    void CheckForEditNode()
    {
        ...
        for (int cIndex = 0; cIndex < EditNodeFolder.childCount; cIndex++)
        {
            ...
            {
                SetPedestrianNode(PedestrianSystem.Tooltip.EDIT,
                    EditNodeFolder.GetChild(cIndex).GetComponent<PedestrianNode>());
            }
        }
    }
