...
public class PedestrianSystem : MonoBehaviour
{
    ...
    public int
        m_objectSpawnCountMax =
            -1; // if -1 then unlimited objects can spawn. If higher than only this amount will ever spawn using the Traffic System spawn options

    public bool m_randomObjectSpawnPerNode = false;
    [Range(0, 1)] public float m_randomObjectSpawnChancePerNode = 0.0f;
    ...
    public List<PedestrianObjectSpawner> m_objectSpawners = new List<PedestrianObjectSpawner>();
    private List<PedestrianObject> m_spawnedObjects = new List<PedestrianObject>();

    public List<PedestrianObject> GetSpawnedObjects()
    {
                ...
                while (perNodeCount < m_numOfObjectsSpawnedPerNode)
                {
                    ...
                    if (rand <= m_randomObjectSpawnChancePerNode && CanSpawn())
                    {
                        ...
                        PedestrianObject obj =
                            Instantiate(m_objectPrefabs[randObjectIndex], transform.position,
                                transform.rotation) as PedestrianObject;
            ...
            case Tooltip.ANCHOR:
            {
                if (TooltipAnchor)
                {
                    ...
                    TooltipAnchor.transform.position = new Vector3(a_obj.transform.position.x,
                        a_obj.transform.position.y + a_obj.GetComponent<Renderer>().bounds.extents.y + 2.0f,
            ...
            case Tooltip.EDIT:
            {
                if (TooltipEdit)
                {
                    ...
                    TooltipEdit.transform.position = new Vector3(a_obj.transform.position.x,
                        a_obj.transform.position.y + a_obj.GetComponent<Renderer>().bounds.extents.y + 2.4f,
                        a_obj.transform.position.z);
                }
            }
                break;
