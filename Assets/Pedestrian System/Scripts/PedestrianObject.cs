...
public class PedestrianObject : MonoBehaviour
{
    ...
    [Range(0.0f, 1.0f)]
    public float
        m_percentageOfSpeedToUse =
            1.0f; // 100% uses the entire values from min to max speed. 34% would only use from min to 35% of max speed. Set this to make objects use a certain amount of the full min / max speed value (currently up to 34% allows for walking animation only) 

    protected float m_speed = 0.0f; // the speed that the pedestrian object will travel at            

    private float
        m_speedStoredWhileWaiting =
            0.0f; // used to record a speed when a node is telling the object to wait ( m_waitAtNode = true )            
    ...
    public float
        m_nodeThreshold =
            0.1f; // how close the pedestrian object needs to get to the m_currentNode before it moves on to another node

    public int
        m_nodeVisitThreshold =
            2; // this is the amount of nodes to remember we have visited so we don't visit the same node again within this threshold

    private PedestrianNode[] m_prevPedestrianNodes;
    private int m_nodeVisitIndex = 0;

    public PathingStatus
        m_pathingStatus =
            PathingStatus.RANDOM; // this determines how the pedestrian object will traverse through the pathing nodes

    public int
        m_pathingIndex =
            0; // if the m_pathingStatus is set to INDEX, then the pedestrian object will try to use this index position for the m_nodes associated in the PedestrianNode.cs object 

    public Vector3 m_offsetPosVal = Vector3.zero; // the amount to offset the position of this object

    public bool
        m_lookAtNode =
            true; // set this to true if you want the objects forward direction to face the node it is currently going towards
    ...
    IEnumerator Start()
    {
        if (PedestrianSystem.Instance)
        {
            ...
            m_lanePosXVariation = Random.Range(-PedestrianSystem.Instance.m_globalLanePosVariation,
                PedestrianSystem.Instance.m_globalLanePosVariation);
            m_lanePosZVariation = Random.Range(-PedestrianSystem.Instance.m_globalLanePosVariation,
                PedestrianSystem.Instance.m_globalLanePosVariation);
        }
        ...
        else if (dir.magnitude > m_nodeThreshold)
        {
            if (GetComponent<Rigidbody>()) // if we have a rigidbody, use the following code to move us
            {
                GetComponent<Rigidbody>().velocity =
                    speed; // set our rigidbody to this speed to move us by the determined speed 

                if (m_lookAtNode)
                    transform.forward =
                        Vector3.Slerp(transform.forward, dir.normalized,
                            m_rotationSpeed *
                            Time.deltaTime); // rotate our forward directoin over time to face the node we are moving towards
            }
            else // no rigidbody then use the following code to move us
            {
                if (GetComponent<Collider>()
                    ...
                    Debug.LogWarning(
                        "Pedestrian System Warning -> Object has a collider. You should think about moving the object with a rigidbody instead.");
            ...
            if (m_onlyRotYAxis)
                transform.rotation =
                    Quaternion.Euler(new Vector3(0.0f, transform.eulerAngles.y,
                        0.0f)); // only rotate around the Y axis.
        }
        else
