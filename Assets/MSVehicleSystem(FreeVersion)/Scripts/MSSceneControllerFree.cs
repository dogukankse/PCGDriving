...
public class MSSceneControllerFree : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        else
        {
            ...
            penaltyAlertText = transform.Find("Canvas/Warnings/warning").GetComponent<Text>();
            speedometer = transform.Find("Canvas/Speedometer").GetComponent<Speedometer>();
            //end transform.find
    ...
    void Update()
    {
        if (!error)
        {
            ...
            if ((Input.GetKeyDown(controls.enterEndExit) || enterAndExitBool) && !blockedInteraction && player &&
                controls.enable_enterEndExit_Input)
            {
                if (vehicles.Length <= 1)
                {
                    if (vehicleCode.isInsideTheCar)
                    {
                        ...
                        {
                            player.SetActive(true);
                            if (vehicleCode.doorPosition[0].transform.position !=
                                vehicles[currentVehicle].transform.position)
                            {
                            ...
                            else
                            {
                                player.transform.position =
                                    vehicleCode.doorPosition[0].transform.position + Vector3.up * 3.0f;
                            }
                        }
                    ...
                    {
                        currentDistanceTemp = Vector3.Distance(player.transform.position,
                            vehicleCode.doorPosition[0].transform.position);
                        for (int x = 0; x < vehicleCode.doorPosition.Length; x++)
                        {
                            proximityDistanceTemp = Vector3.Distance(player.transform.position,
                                vehicleCode.doorPosition[x].transform.position);
                ...
                else
                {
                    ...
                    for (int x = 0; x < vehicles.Length; x++)
                    {
                        ...
                        {
                            currentDistanceTemp = Vector3.Distance(player.transform.position,
                                controllerTemp.doorPosition[y].transform.position);
                            proximityDistanceTemp = Vector3.Distance(player.transform.position,
                                vehicles[proximityObjectIndex].GetComponent<MSVehicleControllerFree>()
                                    .doorPosition[proximityDoorIndex].transform.position);
                    ...
                    else
                    {
                        ...
                        proximityDistanceTemp = Vector3.Distance(player.transform.position,
                            controllerTemp.doorPosition[0].transform.position);
                        for (int x = 0; x < controllerTemp.doorPosition.Length; x++)
                        {
                            currentDistanceTemp = Vector3.Distance(player.transform.position,
                                controllerTemp.doorPosition[x].transform.position);
                            if (currentDistanceTemp < proximityDistanceTemp)
                            {
            ...
            if (vehicles.Length > 0 && currentVehicle < vehicles.Length && UIVisualizer && vehicleCode)
            {
                if (vehicleCode.isInsideTheCar)
                {
                    ...
                    speedometer.SetSpeed(vehicleCode.KMh * clampGear);
